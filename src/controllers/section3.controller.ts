...
import { getQuestionsService, validateAnswersService, getStatsService } from "../services/section3.service";
...
export const getQuestions = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: "Error al obtener las preguntas de Energía Térmica." });
  }
};
...
export const getStats = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: "Error al recopilar las estadísticas." });
  }
};